...
return function(s)
  ...
  local function get_mic_volume()
    awful.spawn.easy_async_with_shell(
      ...
      function(stdout)
        ...
        volume_controller:get_children_by_id("mic_volume_margin")[1].mic_volume.slider_margin.slider:set_value(tonumber(
          volume))
        if volume > 0 then
    ...
    awful.spawn.easy_async_with_shell(
      ...
      function(stdout)
        if stdout:match("yes") then
          volume_controller:get_children_by_id("mic_volume_margin")[1].mic_volume.slider_margin.slider:set_value(
            tonumber(0))
          volume_controller:get_children_by_id("mic_volume_margin")[1].icon:set_image(gears.color.recolor_image(icondir
            .. "microphone-off.svg", color["LightBlue200"]))
        else
  ...
  awesome.connect_signal(
    ...
    function(volume)
      if volume > 0 then
        volume_controller:get_children_by_id("mic_volume_margin")[1].mic_volume.icon:set_image(gears.color.recolor_image(
          icondir
          .. "microphone.svg", color["LightBlue200"]))
      else
        volume_controller:get_children_by_id("mic_volume_margin")[1].mic_volume.icon:set_image(gears.color.recolor_image(
